...
public class Manager : MonoBehaviour
{
	...
	void OnGUI()
	{
		if (atStart)
		{
			...
			if (GUI.Button(new Rect(15, 65, 240, 25), "Move camera"))
			{
				...
				lxfInputFileName = lxfEditor.Edit(lxfInputFileName, true, true);
			}
			...
			if (developerMenu)
			{
				GUI.Box(new Rect(270, 10, 250, 205), "Developer Menu");
				...
				if (GUI.Button(new Rect(275, 155, 240, 25), "Generate LDD color variations"))
				{
					GenerateColorVariations();
				}
				if (GUI.Button(new Rect(275, 185, 240, 25), "Reset PlayerPrefs"))
				{
					Debug.Log("Clearing PlayerPrefs");
					PlayerPrefs.DeleteAll();
				}
	...
	void GenerateColorVariations()
	{
		...
		if (File.Exists(materialsXmlPath))
		{
			List<string> colorsWithVariations = new List<string>(); // List of colors with variations
			XmlDocument materialsXml = new XmlDocument();
			materialsXml.Load(materialsXmlPath);
			XmlNodeList materialNodes = materialsXml.DocumentElement.SelectNodes("//a:Material");
			...
			Debug.Log("Saved " + directoryInfo.FullName + "\\Autogenerated\\Colors with Variations.txt");
			materialsXml.Save(directoryInfo.FullName + "\\Materials edited.xml");
			Debug.Log("Saved " + directoryInfo.FullName + "\\Materials edited.xml");
		}
		else
		{
			Debug.Log("Could not find " + materialsXmlPath);
